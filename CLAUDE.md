...
- `src/git/operations.ts` — `simple-git` + `gh` helpers. Multi-repo aware: each repo in the workspace gets its own branch/commit/PR. `slugifyTask` derives the `agentrium/<slug>` branch name. **Commits are scoped to files agentrium changed, never `git add .`:** at run start the runner takes a content-hash snapshot of each repo's already-dirty files (`snapshotDirty` → path→hash) into `meta.json` (`baselineDirty`, captured once and reused on resume); commit sites stage only `getAgentChangedFiles(repo, baseline)` via `commitChanges(repo, msg, files)`. A file is committed if it was clean at run start (now dirty) **or** was already dirty but its content changed since the snapshot (i.e. an agent edited it) — so agent edits to a pre-existing dirty file are committed, but a dirty file no agent touched is left alone. Don't reintroduce `git add .` in any commit path (runner, review rework, resume).
- `src/git/messages.ts` — LLM-generated PR titles and commit messages, with deterministic fallbacks so generation never breaks a run (uses `src/agents/quickPrompt.ts`, a tool-less one-shot SDK call). **PR title = intent** (`generatePrTitle(task)` → concise English Conventional-Commits title, generated once and cached in `meta.json` as `prTitle`, reused across repos and on resume; fallback = tidied task). **Commit message = actual changes** (`generateCommitMessage(repo, files, {type, stage})` builds the message from the staged diff of exactly the committed files — never restates the task; fallback = neutral `<type>: <stage> changes (N files)`). Don't pass the raw `task` as a PR title or commit subject.
...
4. **Runs are observable and controllable via two files.** Each run dir gets `events.jsonl` (append-only telemetry the run writes; levels `stage ⊂ activity ⊂ raw`) and `control.jsonl` (append-only commands the run reads: `approve/reject/skip`, `pause/resume`, `step_mode_on/off`/`step`, `cancel`). The wire format lives in the `@agentrium/contract` workspace package (`packages/contract/`). `EventLogger` writes events; `ControlChannel` tails commands via `fs.watch` plus a polling fallback and exposes the stage-boundary gate + a cancel `AbortSignal` to the runner. Telemetry/control is non-fatal: a broken file degrades to a no-op, never breaks a run. There is still no server in core — the web UI is a separate `agentrium-ui` consumer of the contract.
